...
import be.vinci.ipl.cae.demo.repositories.StudentBulletinRepository;
...
public class StudentBulletinService {
  ...
  /**
   * Generate bulletins for all students in a class for a specific period. This creates
   * StudentBulletin entities with real calculated grades.
   *
   ...
   */
  @Transactional
  public List<StudentBulletin> generateBulletinsForClass(
      Long classId,
      Long periodId,
      User generatedBy
  ) {
    ...
    Map<Long, Double> classStats = bulletinCalculationService.calculateClassAveragesForPeriod(
        classId,
        periodId
    );
  ...
  public Optional<StudentBulletin> getBulletinByStudentAndPeriod(Long studentId, Long periodId) {
    return studentBulletinRepository.findByStudentIdStudentAndBulletinPeriodIdPeriod(studentId,
      periodId);
  }
  ...
  private String generateDefaultComment(Double average, Student student) {
    ...
    if (average >= 90) {
      return String.format(
        "Excellent travail de %s. Résultats remarquables, continuez ainsi !",
        studentName);
    } else if (average >= 80) {
      return String.format(
        "Très bon travail de %s. Quelques points à améliorer mais très satisfaisant.",
        studentName);
    } else if (average >= 70) {
      return String.format(
        "Travail satisfaisant de %s. "
          + "Des efforts supplémentaires permettraient d'améliorer les résultats.",
        studentName);
    } else if (average >= 60) {
      return String.format(
        "Travail correct de %s. Il est nécessaire de fournir plus d'efforts pour progresser.",
        studentName);
    } else {
      return String.format(
        "%s doit fournir des efforts considérables pour améliorer ses résultats. "
  ...
  public Optional<StudentBulletinDto> getDetailedBulletin(Long studentId, Long periodId) {
    ...
    List<StudentBulletinDto.SubjectGradeDto> subjectGrades = getSubjectGradesForStudent(
        studentId,
        periodId
    );
   ...
   */
  private List<StudentBulletinDto.SubjectGradeDto> getSubjectGradesForStudent(
      Long studentId,
      Long periodId
  ) {
    ...
    List<SubjectCoefficient> subjectCoefficients = subjectCoefficientService.getCoefficientsByClass(
        student.getClassEntity().getIdClass());
  ...
  public List<StudentBulletin> getVisibleBulletinsByUserId(Long userId) {
    ...
    return studentBulletinRepository
      .findByStudentAndIsVisibleOrderByBulletinPeriodBulletinStartDateDesc(
      student, true);
  }
}
