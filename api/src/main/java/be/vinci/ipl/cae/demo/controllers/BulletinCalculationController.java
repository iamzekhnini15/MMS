...
public class BulletinCalculationController {
  ...
  public ResponseEntity<Map<String, Object>> getClassBulletinStatistics(
    ...
    try {
      Map<Long, Double> studentAverages = 
          bulletinCalculationService.calculateClassAveragesForPeriod(classId, periodId);
      Double classAverage = 
          bulletinCalculationService.calculateClassAverageForPeriod(classId, periodId);
      ...
      if (!studentAverages.isEmpty()) {
        double maxGrade = studentAverages.values().stream()
            .mapToDouble(Double::doubleValue).max().orElse(0.0);
        double minGrade = studentAverages.values().stream()
            .mapToDouble(Double::doubleValue).min().orElse(0.0);
        long passCount = studentAverages.values().stream()
            .mapToLong(avg -> avg >= 60 ? 1 : 0).sum();
  ...
  public ResponseEntity<Map<String, Object>> getStudentBulletinStatistics(
    ...
    try {
      ...
      Integer rank = 
          bulletinCalculationService.getStudentRankInClass(studentId, classId, periodId);
      Double classAverage = 
          bulletinCalculationService.calculateClassAverageForPeriod(classId, periodId);
  ...
  public ResponseEntity<Map<String, Object>> getStudentDetailedBulletin(
    ...
    try {
      ...
      Double classAverage = 
          bulletinCalculationService.calculateClassAverageForPeriod(classId, periodId);
      Map<Long, Double> subjectAverages = 
          bulletinCalculationService.getStudentSubjectAveragesForPeriod(studentId, periodId);
