...
# Script de développement pour MMS - Compatible Git Bash Windows
# Usage: ./scripts/dev-bash.sh [command]
...
status() {
    ...
    if docker-compose exec -T database pg_isready -U postgres -d mms > /dev/null 2>&1; then
        log_success "Base de données : OK"
    else
        log_error "Base de données : KO"
    fi
    
    # Vérifier l'API avec retry
    local api_ok=false
    for i in {1..5}; do
        if curl -sf http://localhost:3000/api/actuator/health > /dev/null 2>&1; then
            api_ok=true
            break
        fi
        sleep 2
    ...
    if [ "$api_ok" = true ]; then
        ...
        log_error "API : KO (vérifiez les logs : docker-compose logs api)"
    fi
    ...
    if curl -sf http://localhost:80/health > /dev/null 2>&1; then
        ...
        log_error "Frontend : KO (vérifiez les logs : docker-compose logs frontend)"
    fi
}
...
# Debug - voir les erreurs de build
debug() {
    log_info "Mode debug - construction avec logs détaillés..."
...
help() {
    ...
    echo "  debug     - Build en mode debug avec logs détaillés"
