...
const MyGradesPage: React.FC = () => {
  ...
  const calculateSubjectAverage = (subjectId: number) => {
    ...
    const totalMaxScore = subjectGrades.reduce(
      (sum: number, grade: EvaluationGrade) => sum + (grade.maxScore || 20),
      0,
    );
            ...
            <CardTitle className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
              <div className="flex items-center">
                ...
                <span className="text-base sm:text-lg text-gray-900 dark:text-gray-100">
                  Mes Notes ({filteredGrades.length} évaluation
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-3">
                ...
                <div className="flex flex-wrap gap-1 sm:gap-2">
                  ...
                  >
                    ...
                    {sortBy === 'date' &&
                      (sortOrder === 'desc' ? (
                        <ArrowDownIcon className="w-2 h-2 sm:w-3 sm:h-3 ml-1" />
                      ) : (
                        <ArrowUpIcon className="w-2 h-2 sm:w-3 sm:h-3 ml-1" />
                      ))}
                  </Button>
                  <Button
          ...
          <CardContent className="p-0">
            ...
            <div className="block md:hidden p-4 space-y-3">
              {filteredGrades.map((grade: EvaluationGrade) => (
                <Card
                  key={grade.idGrade}
                  className="border border-gray-200 dark:border-neutral-700 hover:shadow-md transition-shadow bg-white dark:bg-neutral-800"
                >
                  <CardContent className="p-4">
                    ...
                    <div className="flex items-center justify-between mb-3">
                      ...
                      {grade.score !== null && grade.score !== undefined && (
                        <div
                          className={`text-lg font-bold ${getGradeColor(grade.score, grade.maxScore || 20)}`}
                        >
                          {grade.percentage
                  ...
                  <TableRow>
                    ...
                    >
                      <div className="flex items-center">
                        ...
                        {sortBy === 'date' &&
                          (sortOrder === 'desc' ? (
                            <ArrowDownIcon className="w-3 h-3 sm:w-4 sm:h-4 ml-1" />
                          ) : (
                            <ArrowUpIcon className="w-3 h-3 sm:w-4 sm:h-4 ml-1" />
                          ))}
                      </div>
                    </TableHead>
                    <TableHead
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell className="text-right">
                          {grade.score !== null && grade.score !== undefined ? (
                            <div
                              className={`font-semibold ${getGradeColor(grade.score, grade.maxScore || 20)}`}
                            >
                              {grade.percentage
                                ? grade.percentage.toFixed(1)
                                : (() => {
                        ...
                        <TableCell className="min-w-[100px]">
                          {grade.comment ? (
                            <div className="max-w-xs">
                              <p
                                className="text-xs sm:text-sm text-muted-foreground dark:text-gray-400 truncate"
                                title={grade.comment}
                              >
                                {grade.comment}
                              </p>
