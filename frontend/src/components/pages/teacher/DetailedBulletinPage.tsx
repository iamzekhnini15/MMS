...
const DetailedBulletinPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchBulletinData = async () => {
      ...
      try {
        ...
        const data = await getDetailedBulletin(
          parseInt(studentId),
          parseInt(periodId),
        );
  ...
  const generatePDF = () => {
    ...
    doc.text(bulletinData.studentName, pageWidth / 2, yPosition, {
      align: 'center',
    });
    yPosition += 8;
    ...
    doc.setFontSize(11);
    doc.setTextColor(136, 136, 136);
    doc.text(bulletinData.className, pageWidth / 2, yPosition, {
      align: 'center',
    });
    yPosition += 25;
    ...
    doc.text(`${overallAverage.toFixed(1)}%`, pageWidth / 2, yPosition, {
      align: 'center',
    });
    yPosition += 10;
    ...
    const totalPoints = bulletinData.subjectGrades.reduce(
      (sum, subject) => sum + (subject.average * subject.coefficient) / 100,
      0,
    );
    const maxPoints = bulletinData.subjectGrades.reduce(
    ...
    doc.text('Système de Gestion Scolaire', pageWidth / 2, footerY, {
      align: 'center',
    });
    ...
    <div className="container mx-auto px-4 py-4 sm:py-6 max-w-4xl bg-gray-50 dark:bg-neutral-900 min-h-screen">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-0 mb-4 sm:mb-6">
        <Button
          ...
          variant="outline"
          size="sm"
          className="text-sm h-9 self-start sm:self-auto dark:text-white"
        >
        ...
        <Button
          onClick={generatePDF}
          className="bg-green-600 hover:bg-green-700 text-sm h-9 self-start sm:self-auto text-white dark:text-white"
        >
      ...
      <Card className="mb-4 sm:mb-6 bg-white dark:bg-neutral-900 border border-gray-200 dark:border-neutral-800">
        <CardHeader className="text-center px-4 sm:px-6">
          <CardTitle className="text-3xl sm:text-4xl font-light mb-2">
            <span
              className={`${bulletinData.generalAverage >= 50 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}
            >
        ...
        <CardContent className="px-4 sm:px-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
            <div className="text-center p-3 sm:p-0">
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400">
                sur {bulletinData.totalStudents}
              </p>
            </div>
            <div className="text-center p-3 sm:p-0">
              <p className="text-sm text-gray-600 dark:text-gray-400">
              ...
              >
                ...
                <div className="flex items-center justify-between sm:justify-end space-x-3 sm:space-x-4">
                  ...
                  <div className="text-center">
                    ...
                    <Badge
                      className={`text-xs ${subject.average >= 50 ? 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300' : 'bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-300'}`}
                    >
                      {subject.average.toFixed(1)}%
                    </Badge>
                  </div>
          ...
          <div className="mt-4 sm:mt-6 pt-4 border-t border-gray-200 dark:border-neutral-700">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 sm:gap-0">
              ...
              <span className="text-base sm:text-lg font-semibold text-gray-900 dark:text-gray-100">
                {bulletinData.subjectGrades
                  .reduce(
                    (sum, subject) =>
                      sum + (subject.average * subject.coefficient) / 100,
                    0,
                  )
                  .toFixed(1)}{' '}
                /{' '}
                {bulletinData.subjectGrades.reduce(
                  (sum, subject) => sum + subject.coefficient,
                  0,
                )}
              </span>
