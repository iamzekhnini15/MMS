...
const DetailedBulletinModal: React.FC<DetailedBulletinModalProps> = ({
  ...
  const [bulletinData, setBulletinData] =
    useState<DetailedStudentBulletin | null>(null);
    ...
    try {
      ...
      setError(
        err instanceof Error
          ? err.message
          : 'Erreur lors du chargement du bulletin détaillé',
      );
    }
  };
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      pdf.setFontSize(18);
      pdf.setFont('helvetica', 'light');
      pdf.text(bulletinData.studentName, pageWidth / 2, currentY, {
        align: 'center',
      });
      currentY += 8;
      ...
      pdf.setFontSize(12);
      pdf.setTextColor(120, 120, 120);
      pdf.text(`${bulletinData.className}`, pageWidth / 2, currentY, {
        align: 'center',
      });
      currentY += 25;
      ...
      pdf.setFontSize(48);
      pdf.setFont('helvetica', 'light');
      pdf.text(`${generalAvg.toFixed(1)}%`, pageWidth / 2, currentY, {
        align: 'center',
      });
      currentY += 12;
      ...
      pdf.setFontSize(11);
      pdf.setFont('helvetica', 'normal');
      pdf.text('Moyenne générale', pageWidth / 2, currentY, {
        align: 'center',
      });
      currentY += 20;
      ...
      bulletinData.subjectGrades.forEach((subject, index) => {
        ...
        pdf.text(`${subjectAvg.toFixed(1)}%`, pageWidth - 40, currentY, {
          align: 'right',
        });
      ...
      const totalPoints = bulletinData.subjectGrades.reduce(
        (sum, subject) => sum + subject.weightedAverage,
        0,
      );
      const totalCoefficients = bulletinData.subjectGrades.reduce(
      ...
      pdf.setFontSize(9);
      pdf.setFont('helvetica', 'normal');
      pdf.text('Total des points', pageWidth - 40, currentY, {
        align: 'right',
      });
      currentY += 4;
      pdf.setTextColor(0, 0, 0);
      pdf.setFontSize(10);
      pdf.text(
        `${totalPoints.toFixed(1)} / ${(totalCoefficients * 100).toFixed(0)}`,
        pageWidth - 40,
        currentY,
        { align: 'right' },
      );
      ...
      pdf.text(`Généré le ${currentDate}`, pageWidth / 2, footerY, {
        align: 'center',
      });
      pdf.text('Système de Gestion Scolaire', pageWidth / 2, footerY + 5, {
        align: 'center',
      });
        ...
        <DialogContent className="max-w-2xl">
          ...
          <div className="p-4 text-center">
            <p className="text-gray-600 mb-4">
              Aucun bulletin trouvé pour cet étudiant et cette période.
            </p>
            <Button onClick={onClose} variant="outline">
              Fermer
            </Button>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div>
                  ...
                  <Badge
                    className={`text-lg px-3 py-1 ${getGradeColor(bulletinData.generalAverage)}`}
                  >
                ...
                <CardHeader>
                  <div className="flex justify-between items-start">
                    ...
                    <div className="text-right">
                      <Badge
                        className={`text-lg px-3 py-1 mb-2 ${getGradeColor(subject.average)}`}
                      >
                        ...
                        {subject.evaluationGrades.map((evaluation) => (
                          ...
                          >
                            ...
                            <div className="text-right ml-4">
                              <div
                                className={`text-lg font-semibold px-3 py-1 rounded-lg border ${getGradeColor(evaluation.percentage)}`}
                              >
