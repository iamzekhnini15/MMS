...
interface StudentBulletinContextType {
  ...
  getDetailedBulletin: (
    studentId: number,
    periodId: number,
  ) => Promise<DetailedStudentBulletin | null>;
...
const StudentBulletinContext = createContext<
  StudentBulletinContextType | undefined
>(undefined);
...
export const useStudentBulletin = () => {
  ...
  if (!context) {
    throw new Error(
      'useStudentBulletin must be used within a StudentBulletinProvider',
    );
  }
  return context;
};

export const StudentBulletinProvider: React.FC<{
  ...
  const generateBulletinsForClass = async (
    ...
    try {
      const response = await fetch(
        `/api/bulletins/generate/class/${classId}/period/${periodId}`,
        {
          method: 'POST',
          headers: {
      ...
      if (!response.ok) {
        throw new Error(
          `Erreur lors de la génération des bulletins: ${response.status}`,
        );
      }
  ...
  const getBulletinsByClassAndPeriod = async (
    classId: number,
    periodId: number,
  ): Promise<StudentBulletin[]> => {
    setLoading(true);
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Erreur lors de la récupération des bulletins: ${response.status}`,
        );
      }
  ...
  const getBulletinByStudentAndPeriod = async (
    studentId: number,
    periodId: number,
  ): Promise<StudentBulletin | null> => {
    setLoading(true);
    ...
    try {
      const response = await fetch(
        `/api/bulletins/student/${studentId}/period/${periodId}`,
      );
      ...
      if (!response.ok) {
        throw new Error(
          `Erreur lors de la récupération du bulletin: ${response.status}`,
        );
      }
  ...
  const getDetailedBulletin = useCallback(
    async (
      studentId: number,
      periodId: number,
    ): Promise<DetailedStudentBulletin | null> => {
      ...
      try {
        console.log(
          `Fetching bulletin for student ${studentId}, period ${periodId}`,
        );

        const response = await fetch(
          `/api/bulletins/detailed/student/${studentId}/period/${periodId}`,
          {
            method: 'GET',
            headers: {
              'Content-Type': 'application/json',
        ...
        if (!response.ok) {
          throw new Error(
            `Erreur lors de la récupération du bulletin détaillé: ${response.status} ${response.statusText}`,
          );
        }

        const detailedBulletin: DetailedStudentBulletin = await response.json();
        console.log(
          'Bulletin loaded successfully:',
          detailedBulletin.studentName,
        );
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : 'Erreur lors de la récupération du bulletin détaillé';
        setError(errorMessage);
        throw err;
      } finally {
        setLoading(false);
  ...
  const updateBulletinComment = async (
    bulletinId: number,
    comment: string,
  ): Promise<StudentBulletin> => {
    setLoading(true);
    ...
    try {
      const response = await fetch(
        `/api/bulletins/${bulletinId}/comment?comment=${encodeURIComponent(comment)}`,
        {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
      ...
      if (!response.ok) {
        throw new Error(
          `Erreur lors de la mise à jour du commentaire: ${response.status}`,
        );
      }
      ...
      setBulletins((prev) =>
        prev.map((b) => (b.idBulletin === bulletinId ? updatedBulletin : b)),
      );
  ...
  const toggleBulletinVisibility = async (
    bulletinId: number,
  ): Promise<StudentBulletin> => {
    setLoading(true);
    setError(null);
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Erreur lors du changement de visibilité: ${response.status}`,
        );
      }
  ...
  const makeAllBulletinsVisible = async (
    classId: number,
    periodId: number,
  ): Promise<StudentBulletin[]> => {
    setLoading(true);
    ...
    try {
      const response = await fetch(
        `/api/bulletins/make-visible/class/${classId}/period/${periodId}`,
        {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
      ...
      if (!response.ok) {
        throw new Error(
          `Erreur lors de la mise en visibilité: ${response.status}`,
        );
      }
      ...
      const errorMessage =
        err instanceof Error
          ? err.message
          : 'Erreur lors de la mise en visibilité';
      setError(errorMessage);
      throw err;
    } finally {
      setLoading(false);
  ...
  const hideAllBulletins = async (
    ...
    try {
      const response = await fetch(
        `/api/bulletins/hide-all/class/${classId}/period/${periodId}`,
        {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
      ...
      const errorMessage =
        err instanceof Error ? err.message : 'Erreur lors du masquage';
      setError(errorMessage);
      throw err;
    } finally {
      setLoading(false);
  ...
  const bulletinsExist = async (
    classId: number,
    periodId: number,
  ): Promise<boolean> => {
    try {
      const response = await fetch(
        `/api/bulletins/exists/class/${classId}/period/${periodId}`,
      );
