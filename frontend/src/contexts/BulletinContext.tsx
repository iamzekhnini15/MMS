...
interface BulletinContextType {
  ...
  loading: boolean;
  error: string | null;
  fetchBulletinsByClassAndPeriod: (classId: number, periodId: number) => Promise<void>;
...
const BulletinContext = createContext<BulletinContextType>(defaultBulletinContext);
...
export const BulletinProvider = ({ children }: { children: ReactNode }) => {
  ...
  const fetchBulletinsByClassAndPeriod = useCallback(async (classId: number, periodId: number) => {
    setLoading(true);
    setError(null);
    try {
  ...
  const updateBulletinComment = useCallback(async (bulletinId: number, comment: string) => {
    setLoading(true);
    setError(null);
    try {
      const response = await fetch(`/api/bulletins/${bulletinId}?comment=${encodeURIComponent(comment)}`, {
        method: 'PUT',
        headers: {
  ...
  const toggleBulletinVisibility = useCallback(async (bulletinId: number) => {
    setLoading(true);
    setError(null);
    try {
      ...
      const response = await fetch(`/api/bulletins/${bulletinId}?isVisible=${!bulletin.isVisible}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
