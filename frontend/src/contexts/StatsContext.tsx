...
export const StatsProvider = ({ children }: { children: ReactNode }) => {
  ...
  const fetchTeacherStats = useCallback(async (teacherId: number) => {
    ...
    try {
      ...
      const evaluationsResponse = await fetch(`/api/evaluations/teacher/${teacherId}`);
      ...
      const totalStudents = classStats?.reduce((sum, stat) => sum + stat.studentCount, 0) || 0;
      const totalClasses = classStats?.length || 0;
      const averageGrade = classStats?.length ? 
        classStats.reduce((sum, stat) => sum + stat.averageGrade, 0) / classStats.length : 0;
  ...
  const getStudentCountForClass = useCallback((classId: number): number => {
